#!/usr/bin/env python3
"""
Single-agent benchmark that:
  * hydrates archival memory from a Faiss index (like letta_vs_faiss.py),
  * runs search/insert schedules (modes),
  * calls the Letta agent step with retrieved context, and
  * logs per-stage latencies + run averages.
"""
...
try:
    ...
    raise RuntimeError("tqdm is required for letta_agent_benchmark.py. Install it with `pip install tqdm`.") from exc
...
def _assert_postgres_backend() -> None:
    ...
    raise RuntimeError(
        "letta_agent_benchmark.py detected that Letta is configured to use SQLite (settings.database_engine=sqlite). "
...
def format_context(hits: Sequence[Tuple[PydanticPassage, float, dict]], max_chars: int) -> str:
    lines: List[str] = []
    used = 0
...
def build_user_message(query: str, context: str) -> MessageCreate:
    user_text = f"Context:\n{context}\n\nQuestion:\n{query}\n\nAnswer concisely."
    return MessageCreate(role=MessageRole.user, content=user_text)


def extract_assistant_text(messages) -> Optional[str]:
    for msg in messages or []:
        if getattr(msg, "message_type", None) == MessageType.assistant_message:
            ...
            if isinstance(content, list):
                parts: List[str] = []
                for part in content:
                    if hasattr(part, "to_text"):
                        parts.append(part.to_text())
                    else:
...
async def create_benchmark_agent(
    ...
    *,
    ...
    agent_create = CreateAgent(
        name=f"letta-agent-benchmark-{dataset}",
        memory_blocks=[
...
async def hydrate_agent_from_faiss(
    *,
    ...
    batch_starts = list(range(0, total, hydrate_batch))
    for batch_start in tqdm(batch_starts, desc="hydrate", unit="batch", leave=False):
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Benchmark Letta agent step with archival memory search/insert.")
    ...
    parser.add_argument("--faiss-limit", type=int, default=0, help="Limit how many Faiss vectors to hydrate (<=0 means all).")
    ...
    parser.add_argument("--log-jsonl", type=str, default=None, help="Optional JSONL log file path (first 5 queries only).")
    parser.add_argument("--log-csv", type=str, default=None, help="Optional CSV path for per-query timings (all queries).")
    ...
    parser.add_argument("--context-chars", type=int, default=6000, help="Max context size (chars) passed to the agent.")
...
async def main_async() -> None:
    ...
    if hasattr(faiss_index, "make_direct_map"):
        print("[faiss] Building direct map (required for reconstruct)...")
        faiss_index.make_direct_map()

    requested_limit = args.faiss_limit if args.faiss_limit and args.faiss_limit > 0 else None
    ...
    try:
        ...
        if args.log_jsonl:
            ...
            log_path = Path("benchmarks/letta_agent_benchmark/agent_benchmark_first5.jsonl")
        ...
        if args.log_csv:
            ...
            csv_path = Path("benchmarks/letta_agent_benchmark/agent_benchmark_timings.csv")
        ...
        for idx, query in iterator:
            ...
            if do_search:
                ...
                cached_context = format_context(hits, args.context_chars)
            ...
            response = await agent_loop.step(
                ...
                max_steps=1,
                run_id=run.id,
                include_return_message_types=[MessageType.assistant_message],
            )
            t_agent1 = time.perf_counter()
            response_text = extract_assistant_text(response.messages) or ""
            ...
            timings = Timings(
                ...
                agent_ms=(t_agent1 - t_agent0) * 1000.0,
            ...
            if idx < 5:
                print(f"[debug] query[{idx}]={query}")
                if hits:
                    print("[debug] retrieved:")
                    for ridx, (p, score, _m) in enumerate(hits):
                        text = _sanitize_text(getattr(p, "text", "") or "")
                        print(f"[debug]  {ridx+1}. id={getattr(p, 'id', None)} score={score:.4f} text={text[:200]}")
                else:
                    print("[debug] retrieved: (none)")
                print(f"[debug] response[{idx}]={response_text}")
            ...
            if csv_writer:
                csv_writer.writerow(
                    {
                        "query_index": idx,
                        "search_ms": f"{timings.search_ms:.4f}",
                        "agent_ms": f"{timings.agent_ms:.4f}",
                        "insert_ms": f"{timings.insert_ms:.4f}",
                        "total_ms": f"{timings.total_ms:.4f}",
                        "latency_ms": f"{timings.total_ms:.4f}",
                    }
                )
                csv_fp.flush()

...
FIXED: Proper concurrency control to avoid connection pool exhaustion
"""
...
os.environ.setdefault("LETTA_PG_POOL_SIZE", "50")  # Increased to handle concurrency
os.environ.setdefault("LETTA_PG_MAX_OVERFLOW", "30")  # Increased overflow
...
# Sequential DB ops (parallel LLM only)
os.environ.setdefault("LETTA_DB_PARALLEL", "1")
...
async def run_mode(
    *,
    ...
    # db_semaphore removed - all DB ops are sequential
    ...
    async def process_single_query(
        *,
        ...
        # Sequential DB operation - no semaphore needed
    ...
    async def perform_search(batch: SearchBatch, *, do_llm: bool) -> Optional[Sequence]:
        """
        SEQUENTIAL VERSION: Process queries one at a time (no concurrent DB ops).
        LLM calls are still parallelized via llm_semaphore.
        """
        nonlocal ops_in_run
        ...
        for query_index in range(batch.start, batch.end):
...
async def main_async() -> None:
    ...
    print(f"[concurrency] DB parallel={os.environ.get('LETTA_DB_PARALLEL')}, "
          f"Pool size={os.environ.get('LETTA_PG_POOL_SIZE')}, "
          f"Max overflow={os.environ.get('LETTA_PG_MAX_OVERFLOW')}")
    _assert_postgres_backend()

#!/usr/bin/env python
"""
Fixed version that avoids connection pool exhaustion while maintaining DB parallelization.

KEY FIXES:
1. Chunks tasks to match db_parallel limit (never creates more tasks than pool can handle)
2. Properly sized connection pool (pool_size + overflow >= db_parallel)
3. Coordinates semaphore with task creation

This version keeps DB operations parallel but prevents the QueuePool timeout error.
"""
...
# Connection pooling - sized to support parallelism
os.environ.setdefault("LETTA_DISABLE_SQLALCHEMY_POOLING", "false")
os.environ.setdefault("LETTA_DB_PARALLEL", "20")
os.environ.setdefault("LETTA_PG_POOL_SIZE", "30")        # Larger than db_parallel
os.environ.setdefault("LETTA_PG_MAX_OVERFLOW", "15")     # Extra headroom
os.environ.setdefault("LETTA_PG_POOL_TIMEOUT", "120")    # Longer timeout
...
async def run_mode(
    *,
    ...
    async def perform_search(batch: SearchBatch, *, do_llm: bool) -> Optional[Sequence]:
        """
        CRITICAL FIX: Process in chunks that match db_parallel to avoid pool exhaustion.
        
        The key insight: Never create more concurrent tasks than your semaphore allows.
        If db_parallel=20, create max 20 tasks at a time, not 256.
        """
        nonlocal ops_in_run
        ...
        # CRITICAL: Chunk by db_parallel to prevent creating too many waiting tasks
        concurrent_limit = db_parallel
        all_results = []
        
        for chunk_start in range(batch.start, batch.end, concurrent_limit):
            chunk_end = min(chunk_start + concurrent_limit, batch.end)
            tasks = []
            
            # Only create as many tasks as semaphore allows
            for query_index in range(chunk_start, chunk_end):
                local_idx = query_index - batch.start
                text = query_texts[query_index]
            ...
            # Gather this chunk (max db_parallel tasks)
            chunk_results = await asyncio.gather(*tasks) if tasks else []
            all_results.extend(chunk_results)
        
        last_tuples = all_results[-1] if all_results else None
    ...
    async def perform_insert(batch: InsertBatch) -> None:
        ...
        # Use semaphore for insert as well
        async with db_semaphore:
...
async def main_async() -> None:
    ...
    db_parallel = int(os.environ.get("LETTA_DB_PARALLEL", "20"))
    pool_size = int(os.environ.get("LETTA_PG_POOL_SIZE", "30"))
    max_overflow = int(os.environ.get("LETTA_PG_MAX_OVERFLOW", "15"))
    
    print(f"[env] HF cache root set to {hf_root}")
    print(f"[concurrency] DB parallel={db_parallel}, Pool size={pool_size}, Max overflow={max_overflow}, Total capacity={pool_size + max_overflow}")
    
    # Validation: ensure pool can handle parallelism
    total_capacity = pool_size + max_overflow
    if total_capacity < db_parallel:
        print(f"[WARNING] Pool capacity ({total_capacity}) < db_parallel ({db_parallel})")
        print(f"[WARNING] This may cause connection timeouts. Recommend: LETTA_PG_POOL_SIZE >= {db_parallel}")
